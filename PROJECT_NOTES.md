...
## 5.Ц TrendFlow: блок «Hotebook» (Google NotebookLM) — источники+чат+9 артефактов, пер-тенантно (2026-07-05, v1.6.81 → v1.6.85)

> 📖 Полная пошаговая инструкция (установка/подключение/эксплуатация/диагностика) — в [`docs/HOTEBOOK.md`](docs/HOTEBOOK.md). Здесь — конспект для энциклопедии.

**Цель:** управлять Google NotebookLM прямо из TrendFlow. 6-й облачный узел «Hotebook»: **источники** (URL/YouTube/текст/файл из Галереи) → **чат** с цитатами → **студия** из 9 артефактов (аудиопересказ, видеообзор, отчёт, тест, таблица, инфографика, карточки, ментальная карта, презентация) с настройками 1:1 как в NotebookLM (формат-карточки/длина/язык/«акцент»). Готовое → Галерея, вкладка «Hotebook». Индикатор на узле как у подкаста (кольцо генерации + зелёная точка), счётчик генераций за сегодня.

**Почему так, а не OAuth:** у NotebookLM **нет официального API** (есть только Enterprise API в GCloud). Работаем через неофиц. библиотеку `notebooklm-py` (куки сессии). Обычный «Войти через Google» невозможен: у NotebookLM нет OAuth-scope, а сессионные куки (`SID`, `__Secure-1PSIDTS`) — HttpOnly, их не прочитает JS клиента. Настоящего браузерного расширения с cookies-доступом у нас нет (то «расширение» = rehosted-iframe). Поэтому единственный путь — реальный интерактивный вход в браузере.

**Архитектура:** фронт → `apps/backend/src/modules/notebooklm/router.ts` (JWT + Enterprise-гейт как social-ext) → **Hotebook-воркер** `notebooklm-worker/` (FastAPI поверх `notebooklm-py` 0.7.3, домашний ПК/WSL, порт **8802**, по Tailscale) → NotebookLM. Артефакты (mp3/mp4/pdf/png/md/csv/json) скачиваются с воркера в `uploads/hotebook` → `createAsset(folder='hotebook')`. Джобы: таблицы `notebooklm_state`/`notebooklm_jobs` (обе per-tenant), поллинг `GET /jobs/:id` сам финализирует. Адрес воркера — `NOTEBOOKLM_WORKER_URL` (`getNotebookWorkerUrl` в systemConfig + Админ→Конфиг).

**Пер-тенантное подключение (v1.6.84):** профиль notebooklm-py на воркере = `tenantId` (`from_storage(profile=)`), `?profile=<tenantId>` на всех ручках воркера. У каждого Enterprise — СВОЙ Google, свои лимиты и блокноты, изолированы на диске. Импорт сессии — любому Enterprise; окно входа на воркере — только суперадмину; авто-перенос старой платформенной сессии `default` в профиль суперадмина (`adopt-default`).

**Вход прямо в приложении, без файла (v1.6.85):** кнопка «Подключить Google» открывает модалку с ЖИВЫМ окном браузера, стримящимся с воркера — headful Playwright-контекст в профиле тенанта (окно спрятано `--window-position=-2400,-2400`), эндпоинты `/auth/login-remote/{start,frame(JPEG q55),input,poll,stop}`; при появлении кук `SID`+`__Secure-1PSIDTS` на notebooklm.google.com → `ctx.storage_state()` в профиль. Бэкенд проксирует с привязкой `session_id↔tenantId` (тенант рулит только своим окном), `wfetchRaw` для кадров. Фронт: `<img>` тянет кадр через fetch+objectURL (JWT в заголовке — `<img src>` не носит Authorization!), скейл координат по `getBoundingClientRect`, keydown→char/key. Смоук: кадр = реальная страница Sign in Google.

**Параметры генерации сверены с notebooklm-py 0.7.3 интроспекцией** (не по докам!): методы `generate_slide_deck`/`generate_data_table` (не slides/table), enum `AudioFormat{DEEP_DIVE,BRIEF,CRITIQUE,DEBATE}`/`AudioLength`/`VideoFormat`/`VideoStyle`/`ReportFormat`/`SlideDeckFormat`/`QuizQuantity{FEWER,STANDARD}`/`InfographicOrientation/Detail/Style`; чат `ask→AskResult.references`; mindmap отдаёт результат сразу без task_id. Несовпавшие параметры сворачиваются текстом в `instructions` (не роняют генерацию при обновлении либы).

**Инфра (systemd, v1.6.85):** `install.sh` ставит `trendtraffic-notebooklm.service` (автозапуск, `DISPLAY=:0`) + keepalive-таймер, продлевающий сессии ВСЕХ профилей (`refresh-all.sh` по `profiles/*/`). Данные ВНЕ репо: `/opt/tt-hotebook/{home,out}`. **Грабли:** порт 8801 у GPU-студии — Hotebook=8802, `pkill` всегда с портом; install.sh из Windows-чекаута ловил CRLF (`set: pipefail: invalid option name`) → добавлен `.gitattributes` (`*.sh eol=lf`). **Адрес для VPS** = Tailscale-IP хоста `100.122.182.97:8802`, НЕ `0.0.0.0` (в WSL воркер слушает 0.0.0.0, но VPS ходит на Tailscale хоста).
...
## 5.Ч TrendFlow: блок «UGC / Аватары» — удержание, диалоги, вырезка фона, форматы вывода (2026-07-06 … 2026-07-07, v1.6.122 → v1.6.134)

> 📖 Полная дока (интеграции/ключи/пайплайны/эндпоинты/E2E/статус проверки) — в [`docs/UGC_AVATARS.md`](docs/UGC_AVATARS.md). Здесь — конспект для энциклопедии.

**⚠️ НЕ путать с блоком «Аватар» линейного монтажа** (`avatar_step.ts` / `executor_director.ts`, см. «## 6. История фиксов») — тот про говорящую голову ИЗ АУДИО в цепочке OpenMontage (heygen=облако / sadtalker=GPU). Здесь — **отдельное 5-е облако сценария «UGC / Аватары»** (`MontageEditor.tsx`, облако `ugc`): вертикальные/горизонтальные шортсы «аватар + скрипт/запись + видео» → Галерея (`reference`). **Два движка, НЕ смешиваются:** «Своё фото» → **HeyGen** (облако, Avatar IV/III, синхронит губы сам) и «Коллекция» → **SpatialReal** (realtime, сервис `sr-capture` на домашнем ПК, захват с альфой). Общий вход голоса: генерённый скрипт (Claude `generatePodcastDialogue` + озвучка ElevenLabs) ИЛИ разбор записи (`/podcast/diarize`: Gemini → фолбэк воркер, отдаёт `[{speaker:'A'|'B',start,end,text}]`); реплики — в общем `DialogueTimeline`. Все три ветки склейки — в `podcast_compose.ts`; ветвление `/ugc/build` в `render/router.ts`.

**Режим «Удержание» (v1.6.122, задеплоен как 1.6.123; фикс/добивка 1.6.128):** под ОДНУ непрерывную дорожку голоса кадр каждые несколько секунд меняет технику показа, дорогой Avatar IV — только на пики. Только HeyGen «Своё фото». `retention.ts`: пресеты Эконом/Баланс/Премиум (`ivMax` 2/3/4), `planWindows` режет %-окна с притягиванием границ к паузам (`snapToGap`). `director.ts:tagUgcRetention` (Claude) размечает, какие окна поднять на IV (крючок/действие/призыв ≤ ivMax); фолбэк — эвристика `planRetention` (регексы **БЕЗ `\b`** — в JS `\b` не дружит с кириллицей). Аватар: `uploadTalkingPhoto` **один раз** → посегментный `submitTalkingPhotoVideo` (useIV по движку сегмента, аудио = срез записи `sliceAudioToRenders`) → `composeRetentionVideo` (раскладки closeup/split/broll/pip) по КАЖДОМУ B-roll. **Батч:** N видео из Галереи → N роликов, аватар переиспользуется ⇒ цена HeyGen как за один ролик. Ститчер PASS синтетикой.

**Режим «Диалоги» (v1.6.128):** два собеседника, каждый говорит СВОИМ лицом (HeyGen). ГЛАВНОЕ: инфраструктура диалога уже была (diarize отдаёт `speaker`, `PodLine` несёт speaker+image, `DialogueTimeline` двухдорожечный) — UGC лишь СХЛОПЫВАЛ всё в спикера A (`ugcRunDiarize`, правка 1 строки). Тумблер «Диалоги» (при «Своё фото») → разбор держит A/B + фото «Спикер B» + вовлечённость Эконом/Баланс/Динамично. `dialogue.ts:planDialogue`: реплики→сегменты финального таймлайна, склейка соседних реплик одного спикера без медиа в 1 рендер, бюджеты `dlgPreset` (ivMax+twoshotMax). `director.ts:directDialogue` (Claude) per-turn: `engine` iv/iii (крючок/эмоция/призыв), `twoshot` (оба в кадре), `mediaLayout` (только для «Авто»-реплик); фолбэк эвристика. Рендер: два `talking_photo` (A/B), лицевые сегменты → HeyGen со своим срезом записи; **two-shot = говорящий (видео) + реакция второго СТАТИЧНЫМ фото** (без лишнего рендера = дёшево); `buildDialogueVoice` собирает непрерывный мастер-голос (срез записи + звук видео на держании / тишина, каждый кусок `apad+-t` ⇒ сумма = видео, губы не плывут); `composeDialogueVideo` (closeup/twoshot/media-full/media-bg-left|right/media-split). **Медиа умно** (`DialogueTimeline` `dialogueMode`): показ Авто(Клод)/во весь кадр/фон+лицо слева-справа/сверху-снизу; **16:9-медиа = полоса по центру + размытый фон (TikTok), 9:16 = cover**. **Растяжка** «Держать медиа N сек» (видео авто-целиком по длине через `probeMediaDuration`): дольше реплики → медиа держится, последующие реплики СДВИГАЮТСЯ, голос сплайсится. Синтетика PASS (6 раскладок, растяжка +4с, длина=голосу).

**Опциональная вырезка фона аватара (v1.6.132):** тумблер «Вырезать фон аватара» (диалог, раскладка «фон + лицо сбоку» `media-bg-left/right`). Раньше аватар клался непрозрачным прямоугольником со своим фоном. Теперь опц.: `submitTalkingPhotoVideo({bgColor:'#00FF00'})` → HeyGen матирует talking_photo на ЗЕЛЁНЫЙ (`video_input.background` type:color) → `composeDialogueVideo` делает chroma-key + despill (`DlgComposeSeg.avatarChroma`, тот же паттерн, что студийные головы) → чистый силуэт поверх медиа. Только media-bg (closeup/two-shot/split — фон = сцена). **Безопасный фолбэк:** HeyGen проигнорил фон → зелёного нет → chromakey ничего не режет = как прежде (бокс), без вреда. Синтетика PASS (зелёное→медиа navy, красный «человек» остался, утечки нет). Выбор бэкенда вырезки: НЕ render-worker RVM (не хотим привязку к домашнему GPU) и НЕ fal.ai (нет кода) — а зелёнка HeyGen + chromakey (self-contained, cloud, дёшево). Honorит ли HeyGen `background` для talking_photo — подтвердит только живой ключ.

**Формат вывода 9:16 / 16:9 / оба (v1.6.134):** раньше весь UGC жёстко 9:16 (1080×1920). Теперь селектор «Формат вывода» (`spec.formats`). Все ТРИ композитора (`composeUgc`/`composeRetentionVideo`/`composeDialogueVideo`) получили параметр `dims` и стали ориентационными (общие `orientCells`/`placeFilter`/`UGC_FORMATS`): split/two-shot/блоки — портрет верх/низ (**vstack**), ландшафт лево/право (**hstack**); медиа и портретный аватар в «неродной» ориентации — центр + размытый фон (`placeFilter`, бандинг в ОБЕ стороны: 16:9-в-9:16 И 9:16-в-16:9). Роутер: ВСЕ ветки `/ugc/build` гоняют склейку на каждый формат, аватар HeyGen рендерится ОДИН раз ⇒ «Оба» = цена как за один ролик (та же экономика, что батч; `total = brolls × formats`). Готовые файлы с суффиксами `· 9:16` / `· 16:9`. Синтетика PASS (3 композитора × 2 формата = верные размеры, ландшафтный hstack не падает). Урок: 16:9 «двумя числами» не решается — геометрия раскладок ориентационная (vstack↔hstack), но `placeFilter` покрыл и медиа, и портретный аватар в широком кадре одной функцией.

**Экономика движка:** Avatar IV ~$4/мин (пики), Avatar III ~$1/мин (связки), сегмент только медиа/видео и реакция-фото two-shot и держание медиа = **$0**; батч и «оба формата» = аватар 1 раз ⇒ цена как за один ролик. Ролик 1.5 мин на «Балансе» ≈ **$2–3**.

**Ключевые файлы:** бэкенд `render/{router.ts, dialogue.ts, retention.ts, director.ts, avatar.ts, podcast_compose.ts, podcast_voice.ts, audio_diarize.ts}`; фронт `flow/{MontageEditor.tsx, DialogueTimeline.tsx, dialogueTypes.ts}`, `components/GalleryPicker.tsx`. Эндпоинты под `/api/render`: `/ugc/build` (+`/status`), `/ugc/avatars` (list/generate/add/DELETE/spatialreal), `/podcast/diarize`.

**Статус проверки:** планировщики + все склейки + обе ориентации + вырезка фона доказаны **ffmpeg-синтетикой** (без ключей). **НЕ гонялся вживую** (нужен ключ HeyGen клиента + кредиты ~$2–3): реальный рендер диалога/удержания и honorит ли HeyGen зелёный фон для вырезки. Диалог требует записи двух голосов (генерённый скрипт не годится — нет таймкодов); наложенные реплики срезаются до последовательных; SpatialReal-путь требует запущенного `sr-capture` (домашний ПК), HeyGen-пути — полностью облачные. ⚠️ `sr-capture` автозапуск не прописан (после ребута ПК поднимать вручную).

## 5.Ш TrendFlow: блок «Google Flow» (Veo) + Chrome-расширение — двусторонний обмен (2026-07-05 … 2026-07-07, v1.6.90 → v1.6.135, расширение v0.1.0 → v0.2.8)
...
## 5.Э TrendFlow: UGC-СТУДИЯ — полноэкранный редизайн блока UGC + 4-й режим «Без аватара» (2026-07-07 … 2026-07-08, ветка `feat/ugc-studio`, v2.0.6)

**Что это.** Блок UGC (главная карточка сервиса) переехал из модалки 600px в полноэкранную студию по канону видеоредакторов: топбар (выход · автосейв · живая смета · чек-лист-CTA «Создать видео») / панель шагов 1–7 слева / интерактивное превью кадра по центру / док таймлайна снизу. Дизайн-макет утверждался итерациями как интерактивный артефакт (5 версий).

**Фронт (новые файлы `apps/frontend/src/pages/flow/`):** `UgcStudio.tsx` (студия; состояние и хэндлеры остались в MontageEditor, студия — представление через ~25 пропсов), `UgcPreview.tsx` (чистая функция «спека → кадр»: пустые слоты кликабельны, живые субтитры словами первой реплики, кликабельная полоса плана сегментов для Монтажа/Диалога, PNG-слой и полоса прогресса видны как в рендере), `UgcLinesPanel.tsx` (выдвижная панель «Реплики» над доком: фильтр Все/Голос A/Голос B, смена голоса бейджем, объединение ⇣, медиа с подсказкой «покажется целиком: +N с»; авто-открытие при первом появлении реплик), `ugcTypes.ts` (вынесенная спека). «Режим ролика» — производный от спеки: noAvatar→Без аватара, dialogueEnabled→Диалог, retentionPreset≠off→Монтаж, иначе Соло.

**i18n.** ВСЕ ~280 строк студии — в существующем i18next (ns `common`, ключи `ugc.*` в `public/locales/{ru,en}/common.json`; плюралы `_one/_few/_many`). НЕ bespoke-словарь: строки автоматически попадают в конвейер `npm run translate:locales` (~100 языков). Остальные языки до прогона скрипта фолбэчатся на en.

**Новые возможности бэка (`render/podcast_compose.ts` + `router.ts`):**
- Режим «Без аватара — озвучка» (`spec.noAvatar`, `composeVoiceover`): ваше видео во весь кадр + голос (запись как есть или текст→ElevenLabs) + `loudnorm` (ровная громкость записи, `spec.loudnorm`); HeyGen не участвует — цена ≈ только озвучка.
- Форматы `1x1` (1080×1080) и `4x5` (1080×1350) в `UGC_FORMATS`, мультивыбор на фронте. ФИКС `orientCells`: ячейки всегда чётные + stack донормализует scale'ом (4:5 выходил 1080×1348 — crop yuv420p молча резал нечётные 675, и сегменты retention получали разные размеры).
- Музыка: `spec.music.durationSec` — «первые N сек» (вход `-t min(N,D)`) + afade out 1.2с; во всех склейках.
- Заставки до/после: `spec.intro/outro` → `concatBumpers` (нормализация кадра/fps/аудио, у немых бамперов тишина) — во всех 4 ветках.
- Верхний PNG-слой: `spec.layers` (свой на формат) — overlay точно в кадр, ПОД субтитрами; полоса прогресса `spec.progressBar` (drawbox, ширина растёт с t). Вживлено через общий `overlayExtras` в composeUgc/voiceover и финалы retention/dialogue.
- Врезки медиа реплик (`overlay enable='between(t,a,b)'`, ≤12): в соло и озвучке — по таймкодам разбора (l.image + start/end).
- Голоса ElevenLabs: `GET /ugc/voices` (список аккаунта с preview_url, кэш 1ч) + `spec.voiceId` в elevenTTS всех веток.
- Языки серии: `spec.langs` → `translateUgcScript` (Claude, мягкая деградация: язык пропускается с note) → TTS multilingual → файлы «· EN»; в соло HeyGen-голова рендерится на КАЖДЫЙ язык (дороже — фронт предупреждает), в озвучке почти бесплатно.
- Бренд-киты: таблица `brand_kits` (миграция) + `GET/POST/DELETE /ugc/brandkits`; модалка «Применить/Сохранить текущие» (слой+заставки+музыка+субтитры+голос).

**Проверки.** Синтетика ffmpeg реальными композерами: 1:1/4:5/9:16 размеры, музыка-обрезка, voiceover полный+минимальный, заставки (2+5.2+2=9.24с), соло+экстры, retention 4:5+слой — все PASS. tsc бэк/фронт 0, vite build ок. Рантайм-смоук стендом с реальным i18n: RU и EN без сырых ключей во всех режимах. Адверсариал-ревью диффа мультиагентным воркфлоу перед деплоем. ⚠️ Живые прогоны HeyGen/ElevenLabs/Claude-перевода НЕ гонялись (нужны ключи клиента) — как и все прошлые релизы блока.
