...
/* 1.6.84 — НОВЫЙ БЛОК «Hotebook» (Google NotebookLM) в TrendFlow — 6-й облачный узел: источники
 ...
 *         ПОДКЛЮЧЕНИЕ ПЕР-ТЕНАНТНОЕ: каждый Enterprise-аккаунт подключает СВОЙ Google-аккаунт
 *         (профиль notebooklm-py = tenantId на воркере; свои лимиты и блокноты) — статус,
 *         «Проверить», импорт storage_state.json доступны каждому Enterprise; кнопка «Открыть окно
 *         входа Google» (Chromium на машине воркера) — только superadmin, чья это машина; сессия
 *         суперадмина авто-переносится из старого платформенного профиля (adopt-default).
 *         Telegram-алерт владельцу при поломке синхронизации (раз в 12ч, per-tenant). Технически:
 *         notebooklm-worker/ (FastAPI-обёртка notebooklm-py 0.7.3, порт 8802, WSL домашнего воркера;
 *         клиенты и куки per-profile, ?profile= на всех ручках; enum-параметры генерации сверены с
 *         библиотекой, несовпавшие уходят текстом в instructions), бэкенд-модуль
 *         apps/backend/src/modules/notebooklm/ (таблицы notebooklm_state/notebooklm_jobs — обе
