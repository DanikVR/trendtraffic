...
/* 1.6.135 — Google Flow (расширение v0.2.8): картинка «→ Flow» на СПИСКЕ проектов падала «поле
...
/* 2.0.3 — TrendFlow: убраны обе кнопки «Сценарий» (в шапке и центральная «Сценарий ролика —
...
/* 2.0.6 — UGC-СТУДИЯ: полный редизайн блока UGC в полноэкранный редактор (вместо модалки 600px).
 *         (1) Каркас: топбар (смета + чек-лист-CTA «Создать видео»), панель шагов 1–7, интерактивное
 *         превью кадра (пустые слоты кликабельны, живые субтитры из первой реплики, полоса плана
 *         сегментов для Монтажа/Диалога), док DialogueTimeline + выдвижная панель «Реплики»
 *         (фильтр Все/A/B, смена голоса, объединение, медиа с «покажется целиком +N с»).
 *         (2) Режимы явно: Один ведущий / Динамичный монтаж / Диалог двоих / НОВЫЙ «Без аватара —
 *         озвучка» (ваше видео + голос: ИИ-текст ElevenLabs или своя запись как есть + loudnorm;
 *         HeyGen не участвует — почти бесплатно; composeVoiceover + ветка spec.noAvatar).
 *         (3) Новые возможности всех режимов: форматы 9:16/16:9/1:1/4:5 мультивыбором (фикс
 *         orientCells — чётные ячейки, 4:5 выходил 1348px); музыка «весь ролик/первые N сек» с
 *         затуханием; заставки до/после (concat как есть); верхний PNG-слой на формат + полоса
 *         прогресса (под субтитрами); врезки медиа реплик по таймкодам (overlay enable=between);
 *         голоса ElevenLabs из аккаунта с ▶-прослушиванием (GET /ugc/voices + spec.voiceId);
 *         языки серии (перевод Claude → TTS multilingual → «· EN»); бренд-киты (таблица brand_kits
...
/* 2.0.8 — UGC-СТУДИЯ, полировка дизайна по фидбеку: (1) Таймлайн реплик: масштаб до 2000 px/сек
 *         (было 160 — плотные реплики диаризации нечитаемы), зум −/+ держит точку под бегунком,
 *         НОВОЕ Ctrl+колесо = зум к курсору (общий DialogueTimeline — подкаст/Комментатор тоже).
 *         (2) Левая колонка «по-дизайнерски»: пустые слоты (Видеоряд/Музыка/Фото/Запись/Заставки/
 *         Слой) = структурный EmptySlot «иконка → заголовок → подсказка» вместо расползавшейся
 *         строки с иконкой; имена выбранных файлов — 2 строки мелким с обрезом (-webkit-line-clamp)
 *         вместо «Снимок экрана 20…»; кнопки коллекции аватаров переставлены: бриф → широкая
 *         «Сгенерировать 3 аватара» → «Из Галереи» + круглая ↻. (3) Превью на светлой теме:
 *         рамки кадров СВЕТЛЫЕ (CSS-переменные --ugcf-* на .ugc-studio / .dark .ugc-studio),
 *         чёрный «телевизор» остался только в тёмной теме; шахматка слоя тоже темо-зависимая.
 *         i18n: +11 ключей слотов (title/sub) на 108 языков. */
/* 2.0.9 — ГАЛЕРЕЯ-ХАБ, раунд 2 по фидбеку юзера: (1) Блоки открываются ОВЕРЛЕЕМ поверх экрана
 *         (новый FlowBlockOverlay: сам выбирает сценарий — последний/новый/с graph.source — и
 *         рендерит MontageEditor в fixed z-140) — «Назад» возвращает РОВНО туда, откуда пришли:
 *         в тот же раздел Галереи (с перезагрузкой) или в просмотрщик видео. Omni Flash из
 *         VideoViewer — тоже оверлеем ПОВЕРХ редактора (Esc редактора глушится, пока открыт).
 *         (2) Пункт «TrendFlow» УБРАН из меню (сайдбар + мобильный More-sheet): Галерея = главный
 *         экран, всё открывается из неё и возвращается в неё; роут /flow жив для прямых ссылок.
 *         (3) Вкладки Галереи: «Тренды» ПЕРВОЙ (и дефолт), «Из анализа» удалена (разборы живут
 *         в «Тренды»), «TrendFlow»→«Видео» в КОНЦЕ строки с фильтром «Видео | Аудио» (вкладка
 *         «Аудио» слита: kind=reference|audio). (4) Одна кнопка «Медиа» на все типы файлов —
 *         kind по MIME (audio/*→«Аудио», иначе «Видео»), после загрузки открывается нужный
 *         фильтр; описание под заголовком убрано. (5) Вкладка «Тренды»: «Запросы трендов»
 *         СВЕРХУ (первый чип — «+ Добавить тренд»), «Анализ» ниже. Файлы: GalleryPage,
 *         FlowBlockOverlay (новый), VideoViewer, MainLayout, BottomTabBar. */
/* 2.1.0 — КОНВЕЙЕР «ТРЕНД → АВТОАНАЛИЗ → UGC» (автопилот трендов, Enterprise-only).
 *         (1) Автоанализ: блок на странице «Тренды» — ключевик + частота (от часа до недели);
 *         планировщик (trend_watches/auto_runs, тик/мин, переживает рестарт) сканирует по
 *         расписанию, берёт ОДНО новое видео (по новизне; уже разобранные пропускаются, нет
 *         новых → период расширяется лесенкой до «всё время»), скачивает + анализирует;
 *         стоп-краны: интервал ≥ 60 мин, лимит 5/день, автопауза после 3 ошибок.
 *         (2) Серверный Gemini-ВИДЕОАНАЛИЗ ПО КАДРАМ (video_insight, Files API): sceneBeats
 *         теперь реальные, не LLM-реконструкция; вливается в TrendDNA (applyVisualInsight) на
 *         всех путях сохранения анализа. to-gallery вынесен в сервис ingestTrendVideo.
 *         (3) UGC-студия: секция «✨ Анализ тренда» — разбор подмешивается в блоки ГИБКО,
 *         галочками: текст (генерация по формуле+черновику), видео тренда в Видеоряд, стиль
 *         титров, режиссура Монтажа (trendBrief в tagUgcRetention). (4) Видеоряд из ФОТО:
 *         composeSlideshow — 1 фото = статичный кадр, несколько = перелистывание по кругу
 *         (кроссфейд); работает во всех режимах и вручную, и в автопилоте. (5) Шаблоны UGC
 *         (ugc_templates, кнопка «Шаблон»): полный снимок настроек + ключевик-бейдж +
 *         «Включить автопубликацию» (источник: автоанализы Трендов, язык ролика — дефолт EN);
 *         конвейер собирает ролик по шаблону сам (скрипт = copyReadyScript ДНК с переводом).
 *         (6) Галерея → UGC: секция «Авто» — шаблоны с бейджем-ключевиком + авто-ролики
 *         (папка auto-ugc, в «TrendFlow» не дублируются). Скачивание из ленты трендов
 *         осталось БЕЗ анализа (просто в медиа), анализ — по кнопке/автопилотом.
 *         i18n: +47 ключей. Синтетика слайдшоу PASS; живые прогоны — на ключах клиента. */
/* 2.1.1 — ГАЛЕРЕЯ-ХАБ, раунд 3 по фидбэку: (1) СОЛО-РЕЖИМ блоков — панель открывается ЧИСТО
 *         над Галереей/просмотрщиком, БЕЗ холста-паутины и топбара сценария (MontageEditor
 *         soloBlock: холст display:none, верхняя панель скрыта; FlowBlockOverlay прозрачный —
 *         затемняет сам блок). Закрытие панели/студии (×/фон/«Выход») → onBack → возврат РОВНО
 *         откуда пришли (тот же раздел Галереи с перезагрузкой / просмотрщик видео). UGC «Выход»
 *         больше НЕ роняет в сценарий — сразу Галерея → UGC. Omni Flash из просмотрщика — панель
 *         над видео, закрытие → видео. (2) Порядок вкладок: Тренды, UGC, Google Flow, Hotebook,
 *         Видео. (3) «+ Добавить тренд» → Тренды с кнопкой «Закрыть» (возврат в Галерею;
 *         ?from=gallery, флаг залипающий — автоскан чистит query). (4) ИНДИКАТОРЫ ГЕНЕРАЦИИ:
 *         Hotebook — активные джобы (новый GET /api/notebooklm/jobs?active=1) показываются в
 *         Галерее карточкой-плейсхолдером со спиннером «генерируется…», поллинг 4с, по готовности
 ...
 *         пресеты раскладки/кнопка «Сбросить позицию аватара» очищают кастом. Заметность драга:
 *         фиолетовая рамка с белым ободком + значок-пилюля «Двигать» (Move) сверху аватара +
 *         крупный уголок-ручка с иконкой (Maximize2, 20/16px, внутри бокса — не режется кадром) +
 *         строка-подсказка под кадрами; на мини-кадрах фурнитура компактная (иконка без текста). (2) «Пожелания
...
/* 2.2.1 — UGC + Тренды по фидбеку: (1) «+ Добавить» в UGC создаёт НОВЫЙ ролик с именем =
 *         дата и время («09.07.2026 14:30»), дальше переименовывается карандашом. (2) Массовый
 *         выбор + удаление во всех подфильтрах UGC (Ролики/Авто/Макеты): чекбоксы на карточках +
 *         тулбар «Выбрать всё · Удалить (N)». (3) Кнопка «Копировать» на карточке ролика —
 *         дублирует и шаблон, и его сценарий (копия не правит оригинал). (4) Карточки «Тренды →
 *         Анализ» получили массовое удаление (чекбоксы + тулбар + корзина; новые эндпоинты
 *         DELETE/POST /api/trends/analyses). (5) Раздел «Тренды» (Поиск) переведён на раскладку
...
/* 2.2.14 — Галерея по фидбэку: (1) единый ПЛОТНЫЙ вид карточек ВО ВСЕЙ Галерее (как в поиске
 *          трендов): карточка = само изображение (портрет 9/16), сетка до 8 в ряд
 *          (grid-cols-3 sm:5 md:6 lg:8); название, счётчики и ВСЕ иконки-действия НАКЛАДЫВАЮТСЯ
 *          поверх картинки на градиент-скрим (футера под карточкой больше нет) — вкладки Тренды→Анализ,
 *          UGC (Ролики/Авто/Макеты), Google Flow, Hotebook, Видео. (2) На вкладке «Google Flow»
 *          заработал ПОИСК — карточки проектов фильтруются строкой (раньше поиск был мёртвым).
 *          (3) Кнопка «+ Добавить тренд» в Трендах — мерцающая (animate-attract: пульс-свечение +
 *          лёгкое дыхание), привлекает внимание. Файлы: GalleryPage.tsx, tailwind.config.cjs, AppVersion. */
/* 2.2.15 — Галерея по фидбэку (много): (1) в «Видео» добавлена папка «Изображение» → фильтр
 *          Видео/Изображение/Аудио; фильтры + Найдено + Выбрать всё + Удалить + Опубликовать +
 *          Скачать теперь в ОДНУ строчку (на узких — гориз. прокрутка); то же для UGC. (2) Строка
 *          вкладок (Тренды/UGC/Google Flow/Hotebook/Видео/Публикатор) ЛИПКАЯ — всегда сверху при
 *          скролле. (3) Кнопки «Медиа» и «Обновить» из шапки убраны (дублировали «+» и авто-загрузку).
 *          (4) Карточки без изображения — единый плейсхолдер в бренд-индиго (стиль референса): AUDIO —
...
/* 2.2.39 — Hotebook, ЗАГРУЗКА ПО КЛИКУ ИЗ ПЕРЕЧНЯ + видео-фиксы (ext 1.3.27, по решению юзера):
 *          (1) «⬇ В галерею» теперь открывает ПЕРЕЧЕНЬ готовых работ студии в виджете — клик по
 *          работе загружает именно её (✓/«✓ уже»/⚠ на строке); АВТОЗАГРУЗКА ВЫКЛЮЧЕНА (вотчер
 *          оставляет только индикаторы генерации и ⬇TT-кнопки; дедуп сервера не даст задвоить).
 *          (2) Индикатор не ловил ВИДЕОпересказ: его плейсхолдер = «Генерация поясняющего видео.
 *          Это может занять некоторое время» (сущ. «Генерация» + опечатка «пояснящее» в ARTKIND_RE);
 *          добавлены «это может занять»/«генераци», «поясня». (3) 4 копии видео: ОТКРЫТАЯ работа-плеер
...
/* 2.2.43 — Тренды → Аналитика: УДАЛЕНИЕ видео из плитки «Видео из поиска» (frontend-only).
 *          Плитка пустого состояния аналитики (последние source_videos из «Поиска», limit 60)
 *          копилась без возможности чистки. Теперь: корзинка на каждой карточке (ВИДИМАЯ, не за
 *          ховером — канон карточек) + кнопка «Очистить всё · N» в заголовке плитки; обе с
 *          ConfirmModal (danger). Ручки бэка уже были: DELETE /api/trends/videos/:id и
...
/* 2.2.44 — «Умный» пикер Галереи (просьба юзера: «добавляем видео → видим видео, аудио → только
 *          аудио»). GalleryPicker.onlyType принимает и НАБОР типов (['audio','video']); папки, где
 *          нужный тип не бывает, скрываются (фото → без «Аудио»/«Трендов»); если стартовая папка
 *          пуста — автопереход на первую папку с подходящими файлами (пока юзер сам не кликнул);
 *          пустые состояния объясняют, ЧТО сюда подходит. UGC-студия: типизированы ВСЕ цели —
 *          запись=аудио/видео (открывается на «Аудио», загрузка аудио теперь падает в папку
 *          «Аудио», не в «Референс»), медиа реплики=фото/видео, готовое видео-аватар=видео
 *          (+свой заголовок пикера). Также типизированы: старт-кадр Omni (фото), Редактор
...
/* 2.2.50 — UGC-студия: ВСЕ плюсики на превью открывают Галерею и добавляют выбранное.
 *          (1) Бокс аватара: «Готовые аватары» → пикер «добавить из Галереи» (сразу становится
 *          аватаром), «Готовое видео» → пикер видео, «Моё фото» → фото (раньше в соло с
 *          коллекцией/видео клик лишь скроллил левую панель — «ничего не происходило»).
 *          (2) Плюс «Видео» больше не прячется ПОД боксом аватара (бокс ловил клик всей
 *          площадью в раскладках сверху/снизу) — пустое состояние центрируется в свободной
 *          от бокса части кадра; у пустого бокса аватара свой видимый плюс. (3) Плюс «медиа
...
/* 2.3.0  — ПОЛНАЯ ИНТЕРНАЦИОНАЛИЗАЦИЯ + английский как базовый язык перевода. Извлечены ВСЕ
 *          зашитые русские строки фронта (~2170 ключей sec.* из 106 файлов: Тренды/Таргет на ЦА/
 *          Аналитика/Галерея/UGC-студия/MontageEditor/Публикатор/Google Flow/Hotebook/Каналы/
 *          Соцрасширение/админка/биллинг/лендинг/общие компоненты) в t('sec.…','ru-фолбэк').
 *          Попутный аудит текстов: кнопки/подсказки сверены с реальным поведением кода —
 *          исправлено ~50 врущих/устаревших текстов (мёртвые вкладки «Видео»→«Медиафайлы»,
 *          «Omni: генерация — следующий деплой» при живой генерации, тарифы Plus/Standard→Premium,
 *          несуществующие кнопки, dev-жаргон в ошибках, ты→вы). Пайплайн перевода с ПИВОТОМ:
 *          ru → нативный английский (база, translate-pivot.mjs, Gemini 2.5 Flash без thinking)
 *          → все остальные 106 языков С АНГЛИЙСКОГО как носители. Локали common.json полностью
 ...
 *          AppVersion. */
/* 2.3.12 — Галерея → Тренды: ВЕРНУЛИ видимую кнопку добавления тренда — плюс-плиткой
 *          «Добавить» ПЕРВОЙ карточкой в начале обложек (секция «По ссылке»), идентично
 *          остальным вкладкам (UGC/Flow/Hotebook/Медиафайлы). Корень: после 2.2.22
 *          единственная плитка на вкладке жила в секции «Анализ» — уезжала за длинную
 *          «По ссылке» (у юзера 60 видео) и вела в Аналитику, а не в скан. Теперь секция
 *          «По ссылке» видна всегда (даже пустая, в ней плитка), плитка открывает «Тренды»
 *          (скан по ключевику / вставка ссылки) с кнопкой «Закрыть» (?from=gallery);
 *          renderAddTile принял override-действие. Плитка «Анализа» как была (Аналитика).
 *          Тёмная тема по умолчанию ПРОВЕРЕНА фактически на проде (чистый браузер:
 *          app + лендинг стартуют class=dark, фон #0E0E0D/#000) — код не менялся,
 ...
 *          AppVersion. */
/* 2.3.19 — фидбэк по частицам: подложка ПОД контентом везде + ambient-легалки.
 *          (1) Частицы «лезли на текст» карточки MCP и медиа-слотов: слои были
 *          верны (.ttl-fx z0 / .ttl-page z1), но фоны поверхностей были
 *          полупрозрачными (rgba …0.035–0.14) — канвас ПРОСВЕЧИВАЛ сквозь них.
 *          Панель MCP, реплики чата, .ttl-media и оглавление легалок стали
 *          непрозрачными в той же визуальной тональности (#0b0b0d/#120b24/
 *          #160d0a/#0c0c0f) — частицы строго под контентом (в фото-слотах
 *          из 2.3.18 непрозрачный фон = подложка под скриншотами).
 *          (2) /privacy|/terms|/cookies: Constellation получил ambient-режим —
 *          таймлайн от долей полной прокрутки (play→стрелка→play→стрелка,
...
/* 2.5.0 — вики: +2 больших раздела (по слову юзера, программный разбор кода).
 *          (1) «UGC-студия» — 6 тем: устройство студии (шаги 1–7, мультиформат
 *          9:16/16:9/1:1/4:5, ДНК тренда, шаблоны/бренд-кит, чек-лист) и ВСЕ
 *          4 режима ролика (Один ведущий / Динамичный монтаж [eco~2/bal~3/prem~4
 *          крупных плана, план Hook→CTA, батч B-roll] / Диалог двоих [диаризация
 *          A-B, вовлечённость eco/bal/dyn] / Без аватара — озвучка) + шаги 4–7
 *          (видеоряд/слайдшоу, интро-аутро, субтитры, музыка, оверлей, прогресс-
 *          бар); 5 скринов юзера (Content/UGC) в wiki-shots/ugc-*.png.
 *          (2) «Публикатор — автопубликация» — 5 тем: подключение Blotato BYO,
 *          «Новый пост» (лимиты сетей, per-платформа ✎, тред, ИИ-подпись A/B,
 *          платформенные опции, Сейчас/Дата/Следующий слот), слоты (UTC/местное,
 *          горизонт 35 дней), ЦЕПОЧКИ (ручная серия «Опубликовать N» + авто-
 *          цепочка: тик/мин, daily cap 3, формат-фильтр 9:16→TT/16:9→YT,
 *          автопауза 3 фейла, полный конвейер Тренды→шаблон→Авто→цепочка→соцсети),
 *          лента/календарь/ретраи 2-4-8 мин/аналитика. Подпись ссылок в Галерею =
...
/* 2.6.6 — Смета в топбаре UGC считает РЕАЛЬНЫЕ затраты API (просьба юзера
 *          «калькулятор считал правильные затраты на видео через API»).
 *          estimateUgcCost (ugcTypes, чистая функция + юнит-прогон):
 *          · соло «Моё фото»/«Коллекция» по API: Avatar IV $3/мин × ФАКТИЧЕСКАЯ
 *            длительность голоса (ugcScriptSec) × ЧИСЛО ЯЗЫКОВ СЕРИИ (бэкенд
 *            рендерит аватара на каждый язык — раньше смета это игнорировала!)
 *            + ElevenLabs ~$0.2/1000 зн. × языки (ИИ-текст);
 *          · «Диалог двоих»: $3/мин × длительность разобранной записи
 *            (сегменты обоих спикеров суммарно = вся запись);
 *          · «Без аватара» с ИИ-текстом: только ElevenLabs по символам × языки.
 *          Показ: «≈$9.30 — Avatar IV 62 сек × 3 яз. + TTS $0.30»; суффикс
 *          «языков: N» больше не дублируется с расчётом. Пути без чисел —
 *          прежние ориентиры: «Монтаж» (долю лицевых сегментов решает
 *          ИИ-режиссёр на сервере), подписка ext (≈$1/мин), готовое видео $0,
 *          нет реплик. Файлы: ugcTypes.ts, UgcStudio.tsx, locales ru+en. */
/* 2.6.5 — ФИКС: HeyGen-аватар («Моё фото»/«Коллекция», соло) не держал таймлайн
 *          (видео юзера: реплика на 7.5с, а аватар заговорил С НАЧАЛА, видеоряд
 *          обрезался под длину голоса ~2с и сразу заставка). КОРЕНЬ: механизм
 *          «аватар в своём месте таймлайна» (v2.6.2: avatarStartSec/
 *          totalDurationSec в composeUgc) был подключён только к ветке
 *          «Готовое видео». ФИКС в ветке соло-фото: avStart = позиция ПЕРВОЙ
 *          реплики на дорожке (min по lineT: tStart драга ?? start разбора),
 *          длину ролика задаёт видеоряд (хвост голоса продлевает), титры и
 *          врезки медиа реплик сдвигаются на avStart (окно голоса). Без
 *          видеоряда или без сдвига реплик — поведение прежнее (avStart 0).
 *          Файлы: render/router.ts (ветка соло-фото). */
 ...
 *          не дотягивается до чистого зелёного, ключ мажет, despill оставляет
 *          тёмную полупрозрачную муть (репро: фон 1A4D2E у прод-ключа даёт
 *          угол rgb(16,21,97) вместо прозрачного; со СВОИМ ключом — чисто).
 *          ФИКС: detectAvatarBgColor (podcast_compose) — кадр на 0.3с, верхняя
 *          полоса, scale=3:1:flags=area → левая/правая трети; похожи → фон
 *          однотонный, его средний цвет = ключ; пёстрый → null → 0x00FF00 как
 *          раньше. Ветка «Готовое видео» роутера зовёт детектор при галочке
 *          (лог «[ugc/video] chroma-ключ фона:»). despill теперь по доминирующему
 *          каналу ключа (green/blue/никакой) — автоключ может быть не зелёным.
 *          Подсказка галочки: «цвет фона определим автоматически…» (ru+en).
 *          HeyGen-пути (фото/диалог) не тронуты — там фон контролируемый 00FF00.
 *          Файлы: podcast_compose.ts, render/router.ts, locales ru+en. */
/* 2.6.2 — UGC-студия: видео-аватар вставляется в ЛЮБОЕ место таймлайна.
 *          Юзер: «загрузить готового аватара-видео со звуком, вырезать фон и
 *          вставить поверх видеоряда в любое место, как текст в таймлайне»
 *          (AskUserQuestion: хромакей сейчас/ИИ потом · длину задаёт видеоряд ·
 *          двигать + клик=редактор).
 *          1) «Готовое видео» (соло): при заданном видеоряде появляется ОРАНЖЕВАЯ
 *             дорожка «Аватар» на таймлайне — сегмент тащится по времени
 *             (spec.avatarVideoStartSec, выходные сек), клик без сдвига открывает
 *             VideoViewer (обрезка самого видео-аватара, неразрушающе). Панель
 *             показывает длительность аватара (spec.avatarVideoDurationSec из
 *             метаданных; сегмент сам допробирует скрытым <video>).
 *          2) ДЛИНУ РОЛИКА теперь задаёт ВИДЕОРЯД (склейка пре-шага): аватар
 *             появляется и говорит в [start, start+dur], вне окна — только
 *             видеоряд; хвост аватара за концом продлевает ролик. Без видеоряда —
 *             как раньше (длина = длине аватара). composeUgc: avatarStartSec/
 *             totalDurationSec → setpts-сдвиг кадров (в конце цепочки, после fps)
 *             + overlay :enable=between() во всех 4 overlay-ветках; голос —
 *             adelay до старта + apad до конца (amix duration=first не режет).
 *             Вырезка фона — прежний хромакей (галочка); ИИ-матинг любого фона —
 *             отложен отдельной задачей (нужен GPU-воркер).
 *          3) Таймлайн-док живёт и БЕЗ реплик, когда есть клипы видеоряда или
 *             видео-аватар (обрезка/расстановка до текста; у «Готового видео»
 *             текста нет вовсе); блок «Реплики (N)» при пустом диалоге скрыт.
 *             Подкаст/«Комментатор» не затронуты (не передают новые пропсы).
 *          Файлы: ugcTypes.ts, UgcStudio.tsx, DialogueTimeline.tsx (overlayTrack),
 *          MontageEditor.tsx, podcast_compose.ts, render/router.ts,
 *          locales ru+en (+3 ключа). */
...
/* 2.6.42 — ИИ-ПОДПИСЬ: пишется по РАЗБОРУ ролика, а не по имени файла.
 *          Юзер: «вытянуть текст и данные с анализа для подписи, и кнопка "Анализировать"
 *          или "Добавить к тому анализу", который мы хотим».
 *          ⭐ ГЛАВНОЕ: бэкенд это УЖЕ умел (generateCaptions принимал assetId и искал
 *             TrendDNA), но pickFromGallery выбрасывал g.id → assetId всегда пустой,
 *             и разбор не подтягивался НИКОГДА. Одна строка чинит всю цепочку.
 *             Guard: во вкладке «Тренды» g.id = source_videos.id (не media_assets.id),
 *             а при пустом id пикер кладёт туда fileUrl — оба случая дали бы тихий промах.
 *          1) Новый POST /api/trends/media/:id/analyze — разбор СВОЕГО ролика. У него нет
 *             ни ссылки на площадку, ни TikHub-метрик, поэтому порядок обратный: сначала
 *             покадровый Gemini (речь+надписи+сцены), его результат идёт Claude как
 *             «описание» вместо TikHub-summary. Метрики пустые — это честно. Фон + опрос
 *             готовности через GET /media/:id/analysis (job-id в этом конвейере нет).
 *          2) analysisId в /ai-caption + getTrendDNAById: можно взять ЧУЖОЙ разбор —
 *             напр. тренда, под который снят наш ролик. Приоритетнее своего.
 *          3) Контекст подписи расширен: раньше уходили только хук/аудитория/ключи/сценарий.
 *             Добавлены summary, РАСШИФРОВКА РЕЧИ (dna.visual.transcript — то самое «вытянуть
 *             текст»), надписи в кадре, разбор первых секунд, хэштеги оригинала.
 *          4) UI: строка состояния разбора + «Анализировать» + «Взять разбор» (модалка со
 *             списком), плюс честное предупреждение, если модель разбора так и не увидела
 *             (usedDna=false — бэкенд возвращал его и раньше, фронт игнорировал).
 *          Файлы: trends/{router,dna}.ts, media/assets.ts (getAsset), publisher/{service,
 *          router}.ts, PublisherStudio.tsx.
 *          tsc 0×2, vite 0. */
 ...
 *          1) Прошедшие дни в календаре приглушены и БЕЗ кнопки «+» — назад публиковать
 *             некуда. Уже стоящие там посты остаются видны: это история, а не план.
 *          2) У ВСЕХ полей даты (перенос в календаре, дата поста и дата копии в раскрытом
 *             посте, «Дата и время» в композере) появился min = «сейчас»: браузер не даёт
 *             выбрать прошедшее время.
 *          3) Клик по дню ставит 12:00, но если сегодня полдень уже позади — ближайший час
 *             вперёд, иначе композер открывался бы с датой в прошлом.
 *          4) Серверная страховка в submitPost: время в прошлом → понятная ошибка «Время
 *             публикации уже прошло», а не отказ Blotato. Минута форы на дорогу до сервера.
 *          5) «Соцсети» из текста с шевроном превратились в кнопку с рамкой и цветной
 *             плашкой счётчика (зелёная — есть подключённые, оранжевая — ни одной).
 *          Файлы: publisher/service.ts, PublisherTab.tsx, PublisherPostModal.tsx,
 *          PublisherStudio.tsx. tsc 0×2, vite 0. */
/* 2.6.32 — ПУБЛИКАТОР: календарь — главный экран, соцсети свёрнуты.
 *          Юзер: «чтобы календарь был всегда первым и открывался; вторая — галерея;
 *          и кнопка, которая прятала соцсети — нажимаешь, они открываются и подключаются».
 *          1) Вкладка Публикатора открывается на КАЛЕНДАРЕ (было — Лента), и он же первый
 *             в переключателе. Второй — «Галерея» (бывшая «Лента»: она давно стала сеткой
 *             карточек, старое имя врало).
 *          2) Блок «Соцсети» свёрнут по умолчанию: 9 плиток занимали полэкрана над
 *             календарём, а нужны раз в жизни — подключить сеть. Заголовок стал кнопкой
 *             с шевроном и счётчиком «N из 9» (зелёный, если хоть одна подключена);
 *             клик разворачивает плитки, дальше подключение как раньше.
 *          «Обновить» и «Новый пост» остались на виду — они нужны постоянно.
 *          Файлы: PublisherTab.tsx. tsc 0, vite 0. */
 ...
 *          1) Календарь: вместо недели — месяц сеткой 6×7 (всегда 6 строк, чтобы высота
 *             не прыгала), сверху «‹ Июль 2026 ›» + «Сегодня». Хвосты соседних месяцев видно
 *             приглушёнными. В ячейке — компактные чипы постов (значок сети, время,
 *             «M» у ручных), клик раскрывает пост, значок таймера переносит дату.
 *          2) Копия поста: в раскрытом посте «Копировать пост» → плитки всех 9 сетей
 *             (предвыбраны сети оригинала) + дата → POST /manual/duplicate. Копируется
 *             ЛЮБОЙ пост (в т.ч. ушедший в Blotato), но копия всегда ложится в ручной
 *             архив: она никуда не отправляется, пока человек сам не решит.
 *             Текст берётся от сети-донора, а для НОВОЙ сети — от первой строки, чтобы
 *             площадка не осталась с пустым описанием. Чужие accountName/pageId/boardId
 *             в копию не тащим.
 *          Файлы: publisher/{service,router}.ts, PublisherTab.tsx, PublisherPostModal.tsx.
 *          tsc 0×2, vite 0. */
/* 2.6.29 — ПУБЛИКАТОР: лента = сетка карточек как в «Трендах» + календарь из поста.
 *          Юзер (скрины): «должно полностью повторять дизайн как в Трендах, где + это
 *          новый пост; прямо на посте показывать, куда публикуется и дату; нажимаешь —
 *          снизу кнопка "Добавить в календарь" и выбор любой даты».
 *          1) Лента переехала со списка-строк на общий канон Галереи: CARD_GRID
 *             (3/5/6/8 колонок), карточка 9/16 = само видео, текст и ВСЕ кнопки поверх
 *             (без ховера). Первая плитка — «+ Новый пост» с тем же переливающимся
 *             плюсом (animate-attract + animate-iridescent), что и «Добавить» в Трендах.
 *          2) На карточке видно, КУДА публикуется (значки всех сетей поста) и КОГДА
 *             (дата белой плашкой, «без даты» — тёмной). Статус — бейджем сверху слева.
 *          3) В раскрытом посте внизу кнопка «Добавить в календарь» → datetime-picker
 *             (любая дата) + «Убрать дату». Дата ставится на ВЕСЬ пост: все его сети
 *             встают одним днём (wholeGroup).
 *          4) БАГ: у ручного поста статус показывался как «публикуется…» — он никуда не
 *             отправляется. statusChip не знал про 'manual' и проваливался в ветку
 *             по умолчанию. Теперь дата или «без даты».
 *          Правка текста черновиков убрана с карточки — она живёт в раскрытом посте;
 *          заодно вычищен осиротевший код (statusChip, draftEdit, saveDraftText,
 *          toggleDraft). Файлы: PublisherTab.tsx, PublisherPostModal.tsx. tsc 0, vite 0. */
 ...
 *          `scale=-2:bh`, ширина шла из пропорций исходника, а сам силуэт центрировался
 *          по X и прижимался к низу бокса (`overlay=bx+(bw-w)/2 : by+bh-h`). Непрозрачная
 *          ветка при этом считала честный cover-кроп в бокс, как превью, — поэтому баг
 *          вылезал только с включённой вырезкой фона. Замер на ffmpeg (бокс 476×422 @
 *          562,1190): было x 744..853 / y 1278..1541 (мимо бокса), стало x 690..909 /
 *          y 1190..1611 — бокс в пиксель.
 *          Плюс «Динамичный монтаж» и «Диалог двоих» вообще не читали координаты с фронта:
 *          PiP был зашит как 360×640 px в углу (`overlay=W-w-32:H-h-48`, у диалога H-h-140).
 *          В долях кадра это ~0.33×0.33, но в ПИКСЕЛЯХ — поэтому на 16:9 / 1:1 / 4:5
 *          портретный бокс разъезжался, и превью его не показывало (только план-полоска).
 *          Теперь: единая формула бокса `avatarBox(W,H,rect)` (доли → пиксели + cover-кроп
 *          с oy — ровно `object-fit:cover; object-position:50% oy%` из превью) на ВСЕ
 *          режимы, форматы и обе ветки (непрозрачный аватар / силуэт). В «Монтаже» и
 *          «Диалоге» на превью появился тот же перетаскиваемый бокс, что в соло (сегменты
 *          closeup/split/twoshot/media-full — техники показа во весь кадр, бокс к ним не
 *          применяется). Дефолт PiP вынесен в AVATAR_PIP_RECT (фронт+бэк зеркалят друг
 *          друга, как avatarDefaultRect). Кнопка «сбросить позицию» — во всех режимах.
 *          Файлы: render/podcast_compose.ts (avatarBox, AVATAR_PIP_RECT, composeUgc,
 *          overlayAvatarOnVideo, composeRetentionVideo, composeDialogueVideo),
 *          render/router.ts (avatarPipRectFor + прокидка), ugcTypes.ts (AVATAR_PIP_RECT,
 *          avatarRectOf), UgcPreview.tsx, UgcStudio.tsx. */
...
/* 2.6.24 — ПУБЛИКАТОР: РУЧНОЙ АРХИВ. Юзер: «хочу руками добавлять посты, чтобы описание
 *          генерилось, сеть выбиралась ЛЮБАЯ, пост лежал в календаре по датам, и из
 *          раскрытого поста можно было скачать видео и описание — правильный архив для
 *          ручной публикации».
 *          1) Новый статус 'manual': пост живёт ТОЛЬКО у нас, в Blotato не уходит никогда.
 *             Площадка выбирается БЕЗ подключённого аккаунта (account_id='manual:<сеть>'),
 *             ключ Blotato не нужен. Слот-занятость, статус-синк и авторетраи его не видят.
 *          2) Композер «Новый пост»: переключатель «Через Blotato / Вручную». В ручном
 *             режиме — плитки всех 9 сетей, аккаунт не требуется. Текст, который человек
 *             видит в композере, уходит как есть; пустые сети дописывает Пост-движок
 *             (свой текст под каждую сеть), поэтому за уже написанное ИИ не платим.
 *          3) Календарь показывает и очередь Blotato, и ручные посты (значок M).
 *             Клик по дате раскрывает пост.
 *          4) Раскрытый пост (новый PublisherPostModal): плеер, вкладки сетей, правка
 *             текста и заголовка YouTube, «Скачать видео», «Скачать описание» (.txt по
 *             сети, у YouTube с заголовком), «Копировать», дата публикации, удаление.
 *             Открывается и из ленты (обложка/заголовок), и из календаря.
 *          5) Папка «Вручную» в счётчиках ленты.
 *          Файлы: publisher/{service.ts,router.ts}, PublisherPostModal.tsx (новый),
 *          PublisherTab.tsx, PublisherStudio.tsx, docs/PUBLISHER.md. Миграция НЕ нужна:
 *          status — свободный VARCHAR. tsc чист, vite build проходит. */
 ...
 *          и авто-цепочки постили в TikTok/IG/YouTube одинаковый текст, хотя генерация за
 *          него платила. Черновики (createDrafts) при этом работали правильно.
 *          1) captionForItem возвращает весь CaptionVariant; раскладку делает новая
 *             targetsWithCaptions() → textOverride на каждый таргет + заголовок YouTube
 *             (заданный вручную title не перетирается). Обе цепочки переведены на неё.
 *          2) Лимиты длины теперь и на бэкенде (TEXT_LIMITS, копия счётчика Студии):
 *             текст + блок тегов вместе укладываются в лимит сети, обрезка по границе
 *             слова; страховка стоит и в submitPost — цепочки идут мимо UI-счётчика.
 *          3) Хэштеги не дублируются: если модель поставила тег внутрь текста сети,
 *             вторым блоком он уже не клеится.
 *          4) Промпт: при пустом списке площадок больше не просит «заполни ВСЕ: » с пустым
 *             хвостом; префилл '{' + вторая попытка вместо одной — меньше «неразборчивых
 *             ответов»; согласовано «1 вариант/2 варианта».
 *          Файлы: publisher/service.ts, docs/PUBLISHER.md. tsc чист, 19 проверок логики. */
/* 2.6.21 — Разбор речи сам понимает, что голосов ДВА, и раскладывает их по дорожкам.
 *          Жалоба юзера: разобрал запись двух человек (мужчина + женщина) — все 77
 *          реплик легли на дорожку A, дорожка B пустая. КОРЕНЬ (две половины):
 *          1) FE схлопывал разметку: `keepAB = ugc.dialogueEnabled` — вне режима
 *             «Диалог двоих» спикер B принудительно переписывался в A. Бэкенд при
 *             этом делил голоса ПРАВИЛЬНО, результат выбрасывался на клиенте.
 *          2) BE не умел считать голоса: промпт УТВЕРЖДАЛ «запись с ДВУМЯ ведущими»,
 *             поэтому на монологе модель выдумывала второго — автодетект на таком
 *             промпте был бы ложноположительным.
 *          ФИКС: · BE промпт спрашивает, сколько РАЗНЫХ людей говорит (1 или 2), явно
 *            запрещает считать вторым голосом смену интонации/цитирование; схема
 *            ответа расширена { speakerCount, speakers:[{id,gender}] }; число голосов
 *            считается ПО РЕПЛИКАМ (модель врёт speakerCount), одинокий B → A.
 *          · подсказки пола (hostAVoice/hostBVoice) стали НЕобязательными — идут как
 *            «верь своим ушам», а не как факт; FE шлёт их только в готовом диалоге
 *            (в соло ugc.voice — голос ролика, а не пол говорящего на записи).
 *          · FE хранит A/B ВСЕГДА; нашли два голоса → сам включает «Диалог двоих»
 *            (дорожка B рендерится только в нём), пишет это в заметку, чек-лист
 *            дальше просит фото собеседника B.
 *          · Постобработка вынесена в чистую normalizeDiarization + 6 тестов.
 *          Файлы: render/{audio_diarize.ts,router.ts}, tests/diarize.test.ts,
 *          MontageEditor.tsx. */
...
/* 2.6.17 — ПАКЕТ: один сценарий → по ролику на каждое видео Галереи.
 *          Юзер: «20 видео — хочу 20 роликов, меняется только видеоряд, аватар
 *          всегда смеётся в конце». В шаге «Видеоряд» UGC-студии — кнопка
 *          «Пакет: ролик на каждое видео»: мультивыбор в GalleryPicker (до 20) →
 *          панель подтверждения → по задаче POST /ugc/build на каждый источник
 *          (бэкенд не менялся: «Готовое видео» и так сериализуется очередью
 *          ugcVideoChain) → живая панель прогресса со статусом каждой сборки,
 *          ссылкой на готовый ролик и сворачиванием в пилюлю.
 *          КЛЮЧЕВОЕ: галочка «Аватар — в конце каждого ролика» — старт сегмента
 *          считается ПОД КАЖДОЕ видео (avatarVideoStartSec = длина видеоряда −
 *          эффективная длина аватара с учётом обрезки краями), иначе strictDuration
 *          обрезал бы хвост на видео другой длины. Длительности меряются скрытым
 *          <video> (как чипы «Видеоряда»), битый файл не вешает пакет (таймаут 20с).
 *          Рефактор: guard-проверки и сборка спеки вынесены из ugcBuildStart в
 *          ugcBuildProblem()/ugcSpecFor() — одиночная кнопка и пакет ходят одним путём.
 *          Файлы: MontageEditor.tsx, UgcStudio.tsx, ugcTypes.ts, локали ru+en. */
 ...
 *          1) Сегмент «Аватар» теперь тащится на зону концовки (клэмп = видеоряд +
 *             концовка); рендер: если сегмент уезжает за конец видеоряда — аватар
 *             кладётся ФИНАЛЬНЫМ проходом ПОВЕРХ ролика с заставками (новая
 *             overlayAvatarOnVideo: окно в секундах финального видео, интро сдвигает
 *             координаты, голос аватара МИКСУЕТСЯ со звуком заставки — она
 *             продолжает звучать). Внутри composeUgc аватар при этом выключен
 *             (окно за -t, голос 0) — граф не трогаем. Переходы xfade укорачивают
 *             стыки на ~0.25–0.5с — сдвигом пренебрегаем. Чип «После» и её звук
 *             в плее заякорены за видеоряд (аватар их не сдвигает). Наложение
 *             на ИНТРО из UI пока не выражается (нужен сдвиг координат таймлайна) —
 *             отдельным шагом.
 *          2) Карточка «Создаём видео…» в Галерее: таймер mm:ss (ts из
 *             /ugc/build/active), текущий шаг «Склейка · 4/5», серверный статус,
 *             полоса прогресса.
 *          3) Сторожок ffmpeg поумнел: вставший граф ПРОДОЛЖАЕТ печатать stats с тем
 *             же time= (тишины нет — старый детектор не срабатывал) — теперь следим
 *             за ДВИЖЕНИЕМ time=; 120с без движения → SIGKILL с внятной ошибкой.
 *          ГОЧА amix: с базой ПЕРВЫМ входом amix обрубал весь звук на конце короткого
 *          голоса (duration=first не спасал; изолированный репро) — apad-голос строго
 *          ПЕРВЫМ входом, как в composeUgc.
 *          Файлы: podcast_compose.ts (+overlayAvatarOnVideo), render/router.ts,
 *          DialogueTimeline.tsx, GalleryPage.tsx, useUgcActiveBuilds.ts. */
 ...
 *          1) ДЛИНА РОЛИКА СТРОГО = ВИДЕОРЯД: strictDuration в composeUgc (только
 *             ветка «Готовое видео») — хвост аватара за концом обрезается, конец
 *             не «уезжает»; драг сегмента «Аватар» клэмпится в границы видеоряда
 *             (конец сегмента ≤ конец видеоряда); HeyGen-ветки не тронуты (там
 *             хвост голоса продлевает — речь не должна обрываться).
 *          2) КОРЕНЬ «ffmpeg: таймаут» ПРИ ОДИНОЧНОЙ СБОРКЕ: -stream_loop -1 на
 *             клипе флакично ДЕДЛОЧИЛ граф на перемотке (оба обрубка без moov
 *             застыли ровно у конца клипа ~7.1с; репро тех же файлов — секунды).
 *             Клип теперь идёт БЕЗ loop, короткий — замирает на последнем кадре
 *             (tpad=stop=-1:stop_mode=clone в [bg]/[chalf]).
 *          3) Сторожок в ffmpeg(): тишина в stderr 90с (stats молчат = граф встал)
 *             → SIGKILL сразу с ошибкой «ffmpeg завис (нет прогресса 90с)» —
 *             вместо 10-минутного ожидания таймаута.
 *          Файлы: render/podcast_compose.ts, render/router.ts, DialogueTimeline.tsx. */
/* 2.6.9 — Живой скраб превью + индикаторы сборки + очередь рендеров (фикс зависания).
 *          Три просьбы юзера:
 *          1) БЕГУНОК ЛИСТАЕТ ПРЕВЬЮ до готового ролика: клип видеоряда выбирается
 *             по накопленным эффективным длительностям (обрезка+скорость), кадр
 *             сикается на секунду исходника; видео-аватар виден только в своём окне
 *             (вне окна бокс притушен 0.18); плей ▶ тоже ведёт превью (эмит 4 Гц).
 *          2) ИНДИКАТОРЫ СБОРКИ: шаги Очередь→Подготовка→Вырезка→Склейка→Заставки
 *             (по подстрокам серверного статуса; вырезка/заставки — только если есть),
 *             галочки пройденного, таймер mm:ss, полоса прогресса.
 *          3) ФИКС «зависла сборка»: параллельные сборки «Готового видео» душили
 *             друг друга на 2 vCPU до «ffmpeg: таймаут» (живой инцидент: двойной
 *             запуск → два ugcclip за минуту → обе умерли; та же сборка в одиночку
 *             на том же VPS = 17с, локально = 2с — репро на реальных файлах).
 *             Теперь очередь: ugcVideoChain в render/router.ts, статус
 *             «в очереди за предыдущей сборкой…».
 *          Файлы: render/router.ts, DialogueTimeline.tsx, UgcStudio.tsx,
 *          UgcPreview.tsx, locales ru+en (+5 ключей ugc.progress.st*). */
/* 2.6.8 — Таймлайн UGC: заставки видны, плей двух дорожек, ✂ режет видеоряд.
 *          Три просьбы юзера одним сообщением:
 *          1) ЗАСТАВКИ «до/после» показываются на таймлайне (новая серая дорожка):
 *             интро — штриховой маркер у нуля («приклеится ПЕРЕД роликом»),
 *             концовка — сплошной сегмент ПОСЛЕ контента, линейка продлевается
 *             на её длину; длительности probe скрытым video (bumperDur по url).
 *          2) ПЛЕЙ ▶ играет ВСЕ дорожки сразу: звук видеоряда (с учётом галочки
 *             «звук из видео», громкости и скорости ×0,5–×2) + озвучка/аватар
 *             (громкость «Озвучка», трим и позиция сегмента) + концовка своим
 *             звуком; каждому сегменту — свой Audio, «разблокировка» в жесте
 *             клика, старт/пауза в rAF-тике (tlMediaSync). Раньше без реплик
 *             курсор умирал на 1с и не звучало ничего.
 *          3) ✂ по бегунку теперь режет и ВИДЕОРЯД: клип делится на два
 *             независимых сегмента (trimEnd|trimStart в точке реза, секунды
 *             исходника с учётом скорости) — каждый дальше обрезается/
 *             редактируется отдельно; реплики режутся как раньше; максимум
 *             12 кусков (лимит склейки бэкенда).
 *          Файлы: DialogueTimeline.tsx, UgcStudio.tsx, locales ru+en (+6 ключей). */
...
/** 2.6.45 — «Страница, которую читаю → в блокнот» (ext v1.6.4). Единственная дыра Hotebook:
 *          источник добавлялся только изнутри приложения, руками по URL — человек читает статью
 *          и не может закинуть её в работу одним кликом. Появился POPUP у иконки расширения
 *          (manifest.action.default_popup; раньше у action был только тайтл): карточка текущей
 *          страницы, выбор блокнота, «Добавить в блокнот», «Создать блокнот», и второй экран —
 *          МАССОВЫЙ ИМПОРТ (вкладка «Ссылки» — список по одной в строке, вкладка «Вкладки
 *          браузера» — чекбоксы по открытым вкладкам, свои вкладки NotebookLM отфильтрованы).
 *          Новые файлы: src/popup.{html,css,js} (палитра и метрики те же, что у sidepanel.css).
 *          В фоне: popupState/popupRefreshNotebooks/popupCreateNotebook/popupAddSources/
 *          popupBrowserTabs + пять case в шине. Опирается на ГОТОВЫЕ действия content-notebook
 *          (add-source, list-notebooks, create-notebook) — новых селекторов не завёл ни одного.
 *          ⚠️ Наш бэкенд в импорте не участвует: работает, даже если приложение не открыто и
 *          расширение к нему не привязано. ⚠️ Список блокнотов берётся из кэша в chrome.storage,
 *          кнопка ⟳ делает живой скрейп (он уводит вкладку на главную, поэтому во время генерации
 *          запрещён — иначе прервёт её, старые грабли 2.2.x). Добавление строго последовательное:
 *          вкладка одна; прогресс шлётся в popup, но цикл живёт в фоне и доедет, если popup закрыть.
 *          Локали: 36 ключей pop_* в ru и en живым переводом, прочие 50 — английской заглушкой
 *          (скрипт scripts/add-popup-locales.py, append-only).
 *          Тест: test/popup-wiring.test.mjs, 150 проверок на node без браузера — сверяет каждый
 *          $(id) с разметкой, каждый ключ i18n с локалями и каждый тип сообщения с шиной фона.
 *          Проверено, что краснеет: подсунутые опечатки в id и ключе дают 3 провала и выход 1. */
/** 2.6.46 — Popup доведён по итогам адверсариального ревью: 15 находок, 11 подтверждено
 *          проверяющими агентами (ext v1.6.5). ГЛАВНОЕ — ПОРЧА ДАННЫХ: вкладка NotebookLM одна,
 *          а претендентов на неё трое (импорт из popup, генерация, серверный цикл). Второй импорт
 *          уводил вкладку на свой блокнот, а источник кладётся в ТОТ блокнот, что открыт, —
 *          остаток первого пакета молча уезжал в чужой блокнот, и пользователь этого не видел.
 *          ТРИ РУБЕЖА: (1) STATE.tabBusyUntil — общий замок вкладки с дедлайном 120 с, берётся
 *          ДО ensureNotebookTab (иначе вкладку уводит раньше отказа) и продлевается на КАЖДОЙ
 *          итерации; переживает рестарт SW через saveState и сам протухает, а прогресс
 *          (STATE.popupAdding) наоборот НЕ восстанавливается — цикл жил в памяти и умер с воркером;
 *          (2) runNlmAction и _runNlmTask уважают замок и возвращают задачу в очередь
 *          ('busy-import'), как уже делалось для 'busy-generating'; (3) сам addSource в
 *          content-notebook сверяет открытый блокнот с запрошенным и отказывается ('wrong-notebook').
 *          Прочее по ревью: итог пакета уходит ЕЩЁ И широковещательно ('tt-popup-done') — ответ
 *          адресован окну, которое могло закрыться, и переоткрытый popup вис на «20/20» навсегда;
 *          popup подхватывает идущий пакет из состояния и закрывает кнопки; lock(false) больше не
 *          оживляет «Добавить» на chrome:// и на самом NotebookLM (canAddCurrent + try/finally);
 *          выбор блокнота переносится между экранами (иначе 20 источников уезжали в первый по
 *          списку); исчезнувший блокнот откатывает селект вместо пустого поля; разлогин обрывает
 *          пакет с подсказкой войти (ловится и по not-logged-in, и по уходу вкладки с домена);
 *          три ошибки подряд обрывают пакет; отброшенные строки и дубли названы числом;
 *          popupCreateNotebook ставит якорь ttNlmPendingCreate (без него 45 с ожидания и ложное
 *          «не создался» при перезагрузке страницы) и снимает его ИЗБИРАТЕЛЬНО, не затирая чужой.
 *          Плюс страница самого NotebookLM больше не предлагается к добавлению (isNlmUrl).
 *          ГЛАВНОЕ этого раунда — ФУНКЦИОНАЛЬНЫЙ тест popup: test/popup-functional.test.mjs
 *          грузит настоящие popup.html и popup.js в настоящий DOM (jsdom), подставляет заглушку
 *          chrome.* и ДЁРГАЕТ КНОПКИ — 40 проверок поведения, а не разметки: какое сообщение ушло
 *          в фон, что в статусе, что заблокировано, схлопнулись ли дубли, отсеялся ли мусор,
 *          открылся ли пульт Booster с нужным windowId. Проверено, что тест краснеет: отключение
 *          защиты от страницы NotebookLM даёт 4 провала, снятие дедупликации — 2, потеря windowId — 1.
 *          ⚠️ Этому тесту НУЖЕН jsdom (не зависимость проекта): TT_JSDOM=&lt;каталог&gt; node ...
 *          Именно TT_JSDOM, а не NODE_PATH — ESM-резолвер последний игнорирует, jsdom тянется
...
/** v2.6.48 — ext 1.7.0 «Omni 1.1»: пресеты-эффекты + переходы по кадрам в Flow Booster.
 *          Google выкатил Gemini Omni 1.1 Flash в Flow (кейфреймы «первый+последний кадр»,
 *          продление сцены до 40с, 360p-черновики). В пульт Flow Booster добавлены:
 *          1) «Эффекты Omni 1.1» — 10 чипов-пресетов киношных приёмов из анонса (долли-зум,
 *          снэп-зум, орбита 360°, стоп-время, whip-pan, камера в экран, макро-боке, бесшовный
 *          луп, FPV-пролёт, отъезд-раскрытие). Клик вставляет EN-промпт строкой ИЛИ дописывает
 *          хвост эффекта ко ВСЕМ строкам пакета («стилем») — промпты не локализуются, подписи
 *          чипов на 52 языках. 2) «Переходы: первый → последний кадр» (режим «Кадры → Видео») —
 *          пары картинок из панели, строка i → пара i (или один промпт на все; пусто — плавный
 *          переход), content-flow сам кладёт кадры в слоты Flow (токены first/last frame на
 *          9 языках, точный CSS — через flowSelectors.batchCss.frameFirst/frameLast без релиза;
 *          фолбэк — заливка по порядку в общее поле). Провал прикрепления = честный провал item
 *          (не генерим текстом молча). Персонажи к таким item не прикрепляются (слоты заняты).
 *          3) Модель «Omni 1.1 — Flash» в списке + токен 'omni' в modelHint. Тест
 *          sidepanel-wiring.test.mjs: ids/i18n/сообщения/пресеты/токены. Файлы: sidepanel.*,
 *          content-flow.js, manifest 1.7.0, _locales (+34 ключа, append), zip пересобран. */
export const APP_VERSION = '2.6.48';

/** Версия ЕДИНОГО Chrome-расширения TrendTraffic (apps/trendtraffic-extension/manifest.json) —
 *  работает на Google Flow, NotebookLM (Hotebook) и HeyGen. БАМПАТЬ вместе с manifest при каждом
 *  релизе расширения — показывается на карточке «Скачать» в Настройках → «Генерация». */
export const TT_EXT_VERSION = '1.7.0';
