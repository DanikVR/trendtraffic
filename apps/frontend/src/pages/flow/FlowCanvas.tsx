        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '8vh 12px 12px' }} onClick={() => setShowTriggers(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 440, maxHeight: '80vh', overflowY: 'auto', background: 'var(--bg-elevated, #171728)', border: '1px solid var(--border-medium, rgba(255,255,255,0.2))', borderRadius: 12, boxShadow: '0 16px 48px rgba(0,0,0,0.55)', padding: 16 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 10, lineHeight: 1.5 }}>
              {channel === 'tiktok'
                ? <>Бот запускается по триггеру. На TikTok <b>«комментарий» = только публичный ответ</b> (авто-DM из коммента запрещён). <b>«Слово в Директе»</b> — запуск по входящему. Приветствие и рекомендованные вопросы — на странице Botflow (карточка TikTok).</>
                : channel === 'messenger'
                ? <>Бот запускается по триггеру. На Messenger <b>«комментарий → DM»</b> = публичный ответ + приватное сообщение. <b>«Слово в сообщениях»</b> — по входящему. Get Started / приветствие / меню — на странице Botflow (карточка Messenger).</>
