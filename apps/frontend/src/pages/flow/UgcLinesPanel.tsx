...
export default function UgcLinesPanel({ ugc, ugcMutate, onPickMedia, onClose }: UgcLinesPanelProps) {
  ...
  const filters: ['all' | 'A' | 'B', string][] = dialogueMode
    ? [['all', `Все · ${lines.length}`], ['A', `Голос A · ${cnt.A}`], ['B', `Голос B · ${cnt.B}`]]
    : [['all', `Все · ${lines.length}`]];
  ...
  return (
    <div className="absolute left-0 right-0 flex flex-col gap-2 px-3.5 pt-2.5 pb-3"
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <b className="text-[12px]" style={{ color: 'var(--text-primary)' }}>Реплики</b>
        ...
        <span className="text-[10px] ml-auto" style={{ color: 'var(--text-muted)' }}>
          {dialogueMode ? 'бейдж A/B — сменить голос · ⇣ — объединить со следующей' : '⇣ — объединить со следующей'}
        </span>
        <button onClick={onClose} title="Скрыть" className="flex items-center justify-center rounded-lg"
      ...
      <div className="overflow-y-auto flex flex-col gap-1.5 pr-1" style={{ minHeight: 0 }}>
        {lines.map((l, i) => {
          ...
          const holdNote = dialogueMode && vid && hold > dur + 0.05
            ? `видео ${hold.toFixed(1)} с — покажется целиком: +${(hold - dur).toFixed(1)} с, следующие реплики сдвинутся`
            : null;
          return (
            <div key={i} onClick={() => setSel(i)} className="rounded-xl px-2 py-1.5"
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={(e) => { e.stopPropagation(); if (dialogueMode) mutateLine(i, { speaker: sp === 'A' ? 'B' : 'A' }); }}
                  title={dialogueMode ? 'Сменить голос A↔B' : 'Голос ведущего'}
                ...
                {l.image ? (
                  <span className="relative flex-shrink-0" style={{ width: 26, height: 26 }} title={l.imageName || 'медиа реплики — нажмите, чтобы заменить'}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); mutateLine(i, { image: undefined, imageName: undefined, holdSec: undefined }); }} title="Убрать медиа"
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onPickMedia(i); }} title="Прикрепить фото или видео"
                ...
                {i < lines.length - 1 && filter === 'all' && (
                  <button onClick={(e) => { e.stopPropagation(); mergeDown(i); }} title="Объединить со следующей"
                    className="flex-shrink-0 rounded-lg flex items-center justify-center"
                ...
                <button onClick={(e) => { e.stopPropagation(); removeLine(i); }} title="Удалить реплику"
                  className="flex-shrink-0 rounded-lg flex items-center justify-center"
        ...
        {lines.length === 0 && (
          <p className="text-center text-[11px] py-3" style={{ color: 'var(--text-muted)' }}>Реплик пока нет — сгенерируйте текст или разберите запись.</p>
        )}
      </div>
    </div>
  );
