 ...
 *
 * ⚠️ i18n: ВСЕ русские строки в этом файле — СЛОВАРЬ ПАРСЕРА (стемы для матчинга
 * пользовательского ввода RU/EN), а не тексты интерфейса. НЕ оборачивать в t() и
 * НЕ переводить — иначе разбор пожеланий сломается и разъедется с копией на бэке.
 */
