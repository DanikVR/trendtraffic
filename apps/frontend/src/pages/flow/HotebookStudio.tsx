  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="flex items-center gap-2.5 px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-secondary)' }}>
        <button onClick={onClose} title="Выход в Галерею"
        ...
        <input value={hb.name || ''} onChange={(e) => hbMutate((h) => ({ ...h, name: e.target.value }))}
          placeholder="Название блокнота — им подпишутся готовые файлы"
        ...
        <div className="flex items-start gap-2.5 px-4 py-2 flex-shrink-0" style={{ background: 'rgba(245,158,11,0.10)', borderBottom: '1px solid rgba(245,158,11,0.35)' }}>
          ...
          <p className="text-[11.5px]" style={{ color: 'var(--text-secondary)' }}>
            <b style={{ color: '#f59e0b' }}>
              {hbStatus.errorKind === 'ext_offline' ? 'Расширение Hotebook не на связи. ' : hbStatus.errorKind === 'ext_login' ? 'Нужен вход в Google (NotebookLM). ' : 'Синхронизация не работает. '}
            </b>
            {hbStatus.errorKind === 'ext_offline'
              ? 'Установите единое расширение TrendTraffic (Настройки → «Генерация») и откройте вкладку notebooklm.google.com в этом браузере, затем «Проверить».'
              : hbStatus.errorKind === 'ext_login'
                ? 'Откройте notebooklm.google.com и войдите в свой Google-аккаунт, затем «Проверить».'
                : 'Проверьте расширение и открытую вкладку NotebookLM, затем «Проверить».'}
          </p>
        </div>
      )}
      ...
      <div className="flex-1 min-h-0" style={{ display: 'flex', overflow: 'hidden' }}>
        ...
        <section className="flex-1 min-w-0" style={{ display: 'flex', flexDirection: 'column', minWidth: 380 }}>
          <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
            <span className={colHead} style={{ color: 'var(--text-muted)' }}><Sparkles size={13} style={{ color: CYAN }} /> Чат по источникам</span>
            {hb.chat.length > 0 && (
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3">
            {hb.chat.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-6" style={{ color: 'var(--text-muted)' }}>
                ...
                <div className="text-base font-700 mb-1" style={{ color: 'var(--text-secondary)' }}>Спросите о ваших источниках</div>
                <p className="text-[12.5px]" style={{ maxWidth: 460 }}>
                  {noSources ? 'Сначала добавьте источники слева — ссылку, текст или файл из Галереи. Затем задавайте вопросы и собирайте артефакты в Студии справа.'
                    : 'Задайте вопрос по добавленным источникам — NotebookLM ответит с цитатами. Готовые артефакты (аудио, отчёты, тесты…) собираются в Студии справа.'}
                </p>
              </div>
            ) : (
                ...
                {hb.chat.map((m, i) => (
                  <div key={i} className="space-y-1.5">
                    ...
                    <div className="text-[13px] px-3.5 py-2.5 rounded-2xl leading-relaxed" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-subtle)', color: 'var(--text-secondary)' }}>
                      {m.pending
                        ? <span className="inline-flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}><Loader2 size={13} className="animate-spin" /> думает…</span>
                        : <><HbMarkdown text={m.a} />{!!m.cites && <div className="text-[10px] mt-2 font-700" style={{ color: CYAN }}>⌘ {m.cites} цитат из источников</div>}</>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="px-4 py-3 flex-shrink-0 space-y-2" style={{ borderTop: '1px solid var(--border-subtle)', background: 'var(--bg-secondary)' }}>
            ...
            <div className="flex gap-2 items-end" style={{ maxWidth: 760, margin: '0 auto' }}>
              <textarea value={hbChatQ} onChange={(e) => setHbChatQ(e.target.value)} rows={2}
                ...
                placeholder={noSources ? 'Сначала добавьте источники…' : 'Спросите о ваших источниках… (Enter — отправить, Shift+Enter — новая строка)'}
              ...
              <button onClick={() => hbAsk()} disabled={hbChatBusy || !hbChatQ.trim()} title="Спросить"
        ...
        <aside className="p-3.5 space-y-3" style={{ width: 384, flexShrink: 0, borderLeft: '1px solid var(--border-subtle)', overflowY: 'auto', background: 'var(--bg-secondary)' }}>
          <div className="flex items-center justify-between">
            <span className={colHead} style={{ color: 'var(--text-muted)' }}><Wand2 size={13} style={{ color: CYAN }} /> Студия</span>
            <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Сегодня: {genToday} генераций</span>
          </div>

          {/* 9 плиток */}
          <div className="grid grid-cols-2 gap-2">
            {hbTypes.map(({ t, label }) => {
              const made = hbArtifactCounts[t] || 0;
              const gen = hbJobs.some((j: any) => j.type === t && (j.status === 'queued' || j.status === 'running'));
              return (
                <button key={t} onClick={() => hbOpenGen(t)} disabled={noSources}
                  title={noSources ? 'Сначала добавьте источники' : `Настроить и сгенерировать: ${label}${made ? ` · сделано: ${made}` : ''}`}
                  ...
                  <span style={{ color: CYAN }}>{hbIcon[t]}</span>
          ...
          <div className="pt-1">
            ...
            {hbJobs.length === 0 ? (
              <div className="rounded-xl p-4 text-center" style={{ background: 'var(--bg-tertiary)', border: '1px dashed var(--border-medium)' }}>
                ...
                <p className="text-[11.5px]" style={{ color: 'var(--text-muted)' }}>Готовые аудио, видео, отчёты и другое появятся здесь. Также — в Галерее → «Hotebook».</p>
              </div>
            ) : (
              <div className="space-y-1.5">
                {hbJobs.map((j: any) => {
                      ...
                      <div className="flex items-center gap-2 px-2 py-1.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-[9px]" style={{ color: j.status === 'error' ? '#ef4444' : 'var(--text-muted)' }} title={j.error || undefined}>
                            {running ? 'Генерируется… (можно закрыть студию)' : j.status === 'error' ? (j.error || 'Ошибка') : `Готово · ${new Date(j.createdAt).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}`}
                          </div>
                        </div>
                        ...
                        {j.status === 'done' && isJson && j.payload && (
                          <button onClick={() => setHbView(j)} className="text-[10px] font-700 px-2 py-1 rounded-lg flex-shrink-0"
                            style={{ background: CYAN, color: '#083344', border: 'none', cursor: 'pointer' }}>Открыть</button>
                        )}
                        {j.status === 'done' && j.fileUrl && !isJson && !playable && (
                          <a href={j.fileUrl} target="_blank" rel="noreferrer" className="text-[10px] font-700 px-2 py-1 rounded-lg flex-shrink-0"
                            style={{ background: CYAN, color: '#083344', textDecoration: 'none' }}>Открыть</a>
                        )}
