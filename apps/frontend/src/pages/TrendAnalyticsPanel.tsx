    ...
    <div className="space-y-5">
      ...
      {result && (
        <>
          ...
          {/* Транскрибация речи → перевод → Галерея «Текст» → озвучка UGC (общий компонент) */}
          {isVideo && <TranscribePanel token={token} url={url.trim()} platform={result.detected.platform} />}
