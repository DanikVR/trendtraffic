...
## Эффекты Omni 1.1 и переходы по кадрам (1.7.0)

В Flow Booster два инструмента под Gemini Omni 1.1 (модель Flow с 08.2026):

- **«Эффекты Omni 1.1»** — 10 чипов-пресетов киношных приёмов из анонса Google
  (долли-зум, снэп-зум, орбита 360°, стоп-время, whip-pan, камера в экран,
  макро-боке, бесшовный луп, FPV-пролёт, отъезд-раскрытие). Клик вставляет
  готовый EN-промпт новой строкой ИЛИ дописывает хвост эффекта ко всем строкам
  пакета («стилем»). Тексты промптов не локализуются (модели понимают EN),
  подписи чипов — на 52 языках. Данные — `OMNI_PRESETS` в `sidepanel.js`.
- **«Переходы: первый → последний кадр»** (режим «Кадры → Видео») — пары
  картинок в панели; строка промпта i → пара i (или один промпт на все пары;
  пусто — вставится плавный переход). `content-flow.js` кладёт кадры в слоты
  Flow по токенам `frameSlotTokens` (first/last frame на 9 языках); точный CSS
  можно задать без релиза: `chrome.storage.local['flowSelectors'] =
  {batchCss:{frameFirst:'…', frameLast:'…'}}`. Слоты не нашлись — фолбэк:
  заливка по порядку в общее поле. Провал прикрепления = провал item (текстом
  молча не генерим). Персонажи к таким item не прикрепляются — слоты заняты.

Проводка обеих фич закрыта тестом `test/sidepanel-wiring.test.mjs` (без браузера).
