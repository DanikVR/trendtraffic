...
async function personCutoutGreen(apiKey: string, groupUrlAbs: string, side: 'A' | 'B', box?: FaceBox | null, strict = false): Promise<{ url: string; path: string }> {
  ...
  const gen = await generateImage({
    ...
    prompt: `Оставь на изображении ТОЛЬКО человека ${where}. Полностью убери второго человека и всю студию/фон. `
      ...
      + 'Человек ПОЛНОСТЬЮ НЕПРОЗРАЧНЫЙ и плотный: сквозь него НЕ должен просвечивать зелёный фон, '
      + 'никакой полупрозрачности или смешивания человека с фоном. '
      + 'Фотореалистично. Верни только изображение.'
      // повторная попытка: прошлый результат — не зелёный фон ИЛИ полупрозрачный человек
      + (strict
        ? ' ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ: итоговое изображение — это ОДИН ПОЛНОСТЬЮ НЕПРОЗРАЧНЫЙ человек на '
          + 'СПЛОШНОМ ярко-зелёном фоне #00FF00. НИКАКОЙ студии, мебели, столов, экранов, микрофонов и '
          + 'деталей интерьера на фоне быть НЕ ДОЛЖНО. Если фон не зелёный ИЛИ человек хоть частично '
          + 'прозрачен (сквозь него виден фон) — результат НЕВЕРЕН.'
        : ''),
    inputImages: [{ base64: img.base64, mime: img.mime }],
  });
...
router.post('/podcast/heygen-studio', async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const prepared = await Promise.all(hostsList.map(async ([spk, host]) => {
      ...
      // Валидация вырезки ДО HeyGen (кредиты не тратятся), два режима сбоя Gemini:
